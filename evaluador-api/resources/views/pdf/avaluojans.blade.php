...
<div class="page-break"></div> <!-- salto a nueva hoja -->

<!-- METODOLOGÍA VALUATORIA -->
<div style="margin-top: 20px;">
    <h3 style="background: #c00; color: #fff; margin: 0; padding: 6px; font-size: 14px; text-align: center;margin-top: 10px;">
        Metodología Valuatoria
    </h3>

    <div style="font-size: 12px; text-align: justify; margin-top: 8px;">
        <strong>4.2 Método del Mercado</strong><br>
        El método usado para el cálculo de datos es el de comparación de mercado. 
        Este método consiste en reunir datos de varias fuentes del mercado local como lo son: 
        concesionarios de nuevos y usados, clasificados en sitios web, 
        revistas especializadas e información histórica de inspecciones y avalúos.
    </div>
    ...
    <!-- Texto al pie del recuadro -->
    <div style="font-size: 11px; text-align: justify; margin-bottom: 20px;">
        El gráfico corresponde a las fuentes tenidas en cuenta como lo son: revistas especializadas, 
        clasificados de internet, para la estimación del valor razonable para el vehículo {{$ingreso->marca}} {{$ingreso->linea}} {{$ingreso->modelo}}
        en condiciones normales de operación y documentación vigente.
    </div>
</div>
    ...
    <table style="width:100%; border-collapse:collapse; text-align:center;">
        <tr>
            ...
            <td style="width:50%; padding:8px; vertical-align:top;">
                <div style="border:1px solid #000; height:190px; overflow: hidden;">
                    @foreach($ingreso->images as $imaged)
                        @if($imaged->categoria == 'derecha')
                            @php
                                $pathd = public_path($imaged->path);
                                $typed = pathinfo($pathd, PATHINFO_EXTENSION);
                                $datad = file_get_contents($pathd);
                                $base64d = 'data:image/' . $typed. ';base64,' . base64_encode($datad);
                            @endphp
                            <div style="width:100%; height:190px; 
                ...
                <div style="background:#d71920; color:#fff; font-weight:bold; padding:4px; margin-top:2px;">Derecha 2</div>
            </td>
        </tr>
        <tr>
            <td style="width:50%; padding:8px; vertical-align:top;">
            ...
            <td style="width:50%; padding:8px; vertical-align:top;">
                <div style="border:1px solid #000; height:190px; overflow: hidden;">
                    @foreach($ingreso->images as $imaget)
                        @if($imaget->categoria == 'trasera')
                            @php
                                $patht = public_path($imaget->path);
                                $typet = pathinfo($patht, PATHINFO_EXTENSION);
                                $datat = file_get_contents($patht);
                                $base64t = 'data:image/' . $typet . ';base64,' . base64_encode($datat);
                            @endphp
                            <div style="width:100%; height:190px; 
                                        background-image: url('{{ $imaget->path }}');
                                        background-size: cover;
                                        background-position: center center;">
                            </div>
                            @break
